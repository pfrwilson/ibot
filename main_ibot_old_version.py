...
def get_args_parser():
    ...
    parser.add_argument('--arch', default='vit_small', type=str,
        choices=['vit_tiny', 'vit_small', 'vit_base', 'vit_large', 'deit_tiny', 'deit_small',
                 'swin_tiny','swin_small', 'swin_base', 'swin_large', 'medsam'],
    ...
    parser.add_argument("--do_unsupervised_eval", default=False, help='Whether to perform unsupervised evaluation, clustering cls token -> image folder labels', action='store_true')
    parser.add_argument("--do_nct_probing", default=False, help='Whether to perform probing on the NCT micro-ultrasound dataset', action='store_true')
    parser.add_argument("--wandb_run_id", default=None)

    # Data parameters
    parser.add_argument('--global_crops_size', default=224, type=int, help='Size of global crop.')
...
def train_ibot(args):
    ...
    if utils.is_main_process():  # wandb
        run = wandb.init(project="iBOT", config=args, id=args.wandb_run_id, resume='allow')
    ...
    if args.do_nct_probing:
        ...
        nct_probe = NCTProbing(args.batch_size_per_gpu, size=args.global_crops_size)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if args.do_nct_probing:
            ...
            probing_results = nct_probe.run_probing(
                teacher_without_ddp, epoch, 'cuda', is_main_process=utils.is_main_process(), 
            )
...
def train_one_epoch(
    ...
    for it, (images, labels, masks) in enumerate(
        ...
        if any([
            not math.isfinite(loss_.item()) for loss_ in losses_across_batch
        ]):
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser("iBOT", parents=[get_args_parser()], formatter_class=argparse.ArgumentDefaultsHelpFormatter)
