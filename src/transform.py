...
class RandomNoise: 
    ...
    def __call__(self, img):
        if self.type == 'speckle':
            return SpeckleNoise(np.random.uniform(*self.amount_range), self.apply_to_greyscale)(img)
        elif self.type == 'salt_and_pepper':
            return SaltAndPepperNoise(np.random.uniform(*self.amount_range), self.apply_to_greyscale)(img)
        else:
    ...
    def __post_init__(
        ...
        if self.initial_crop_size is not None: 
            if self.initial_crop_scale is not None: 
                ...
                self.initial_crop = T.Compose([T.RandomCrop(self.initial_crop_size), T.Resize(self.initial_crop_size)])
        else:
        ...
        strong_augs = T.Compose(
            [
                T.RandomApply([RandomNoise(apply_to_greyscale=True, type='speckle')], p=self.speckle_prob),
                T.RandomApply([RandomNoise((0.01, 0.1), apply_to_greyscale=True, type='salt_and_pepper')], p=self.salt_and_pepper_prob),
